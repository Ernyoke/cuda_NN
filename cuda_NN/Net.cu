...
void Net::Train_GPU(const double* inputs, const double* targets)
{
	//let the input flow throught the net
...
const double* Net::feedForward_GPU(const double* inputs)
{
	//get the first layer from the list and feed the input into it
	auto i = 0;
	auto firstLayer = layers.at(i++);
	firstLayer->FeedForward_GPU(inputs);
	const double* output = firstLayer->Output();
...
void Net::backPropagate_GPU(const double* targets)
{
	//get the last layer from the list
	int i = layers.size() - 1;
	auto lastLayer = layers.at(i--);
