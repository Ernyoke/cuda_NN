...
int main()
{
	...
	inputLayer->InitWeights();
	std::shared_ptr<Layer_GPU> hiddenLayer(new Layer_GPU(1, 10));
	hiddenLayer->InitWeights();
	std::shared_ptr<Layer_GPU> hiddenLayer2(new Layer_GPU(10, 10));
	hiddenLayer2->InitWeights();
	std::shared_ptr<Layer_GPU> hiddenLayer3(new Layer_GPU(10, 10));
	hiddenLayer3->InitWeights();
	std::shared_ptr<Layer_GPU> outputLayer(new Layer_GPU(10, 1));
	outputLayer->InitWeights();
